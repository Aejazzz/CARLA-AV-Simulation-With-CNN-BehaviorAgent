...
# ============================================================
#   MINIMAP RENDER FUNCTION  (paste above game_loop)
# ============================================================
def render_minimap(world, agent, size=(420, 420)):
    """
    Draws a Google-Maps style minimap with the agent's global route.
    """

    # create empty minimap
    minimap = pygame.Surface(size)
    minimap.fill((245, 245, 245))

    # get map & spawnpoints
    carla_map = world.map
    spawn_points = carla_map.get_spawn_points()

    # ----- Build conversion from world → minimap pixels -----
    ...
    padding = 30
    scale = min((size[0] - 2*padding) / w, (size[1] - 2*padding) / h)

    def w2m(loc):
        px = padding + (loc.x - minx) * scale
        py = size[1] - (padding + (loc.y - miny) * scale)
        return (int(px), int(py))

    # ----- Draw the global route (blue line) -----
    try:
        route = agent._global_plan   # BehaviorAgent global plan
        route_points = [wp[0].transform.location for wp in route]

        if len(route_points) >= 2:
            pts = [w2m(p) for p in route_points]
            pygame.draw.lines(minimap, (40, 120, 230), False, pts, 4)

            # small white waypoints
            for p in pts:
                pygame.draw.circle(minimap, (255, 255, 255), p, 3)

    except:
        pass

    # ----- Draw ego vehicle -----
    ego = world.player.get_location()
    ego_px = w2m(ego)

    pygame.draw.circle(minimap, (250, 40, 40), ego_px, 6)
...
# -----------------------
# Minimap save utilities
# -----------------------
def create_minimap_surface(minimap_size=(512, 512), background=(240,240,240)):
    """Create empty minimap surface of chosen size (RGB)."""
    surf = pygame.Surface(minimap_size)
    surf.fill(background)
    return surf

def draw_route_on_minimap(minimap_surf, world_to_minimap, route_points, ego_point=None):
    """
    Draw route_points on minimap_surf.
    world_to_minimap: fn that converts carla.Location -> (x,y) on minimap_surf
    route_points: list of carla.Location or (x,y)
    ego_point: optional (x,y) to draw ego marker
    """
    if len(route_points) < 1:
        return
    # draw polyline (Style 1)
    pts = [world_to_minimap(p) if hasattr(p, 'x') else p for p in route_points]
    if len(pts) >= 2:
        pygame.draw.lines(minimap_surf, (42, 130, 218), False, pts, 4)  # blue route
    # draw waypoints small circles
    for p in pts:
        pygame.draw.circle(minimap_surf, (255,255,255), (int(p[0]), int(p[1])), 3)
    # ego marker
    if ego_point is not None:
        pygame.draw.circle(minimap_surf, (220,50,50), (int(ego_point[0]), int(ego_point[1])), 6)

def save_minimap_png(surface, filename):
    """Save pygame surface to PNG file on disk."""
    try:
        pygame.image.save(surface, filename)
        print(f"[MINIMAP] Saved PNG -> {filename}")
    except Exception as e:
        print("[MINIMAP] Failed saving PNG:", e)


def make_world_to_minimap_fn(spawn_points, minimap_size=(512,512), padding=32):
    """
    Build a closure that maps carla.Location -> (px,py) in minimap surface.
    Uses bounding box of spawn_points to cover whole map area (simple method).
    """
    xs = [p.location.x if hasattr(p, 'location') else p.x for p in spawn_points]
    ys = [p.location.y if hasattr(p, 'location') else p.y for p in spawn_points]
    minx, maxx = min(xs), max(xs)
    miny, maxy = min(ys), max(ys)
    ...
    # add padding inward
    map_w = minimap_size[0] - 2*padding
    map_h = minimap_size[1] - 2*padding
    scale = min(map_w / width, map_h / height)

    def world_to_minimap(loc):
        # map carla coordinates to minimap pixel coords (y-axis inverted)
        px = padding + (loc.x - minx) * scale
        py = minimap_size[1] - (padding + (loc.y - miny) * scale)  # invert y for display
        return (int(px), int(py))

    return world_to_minimap
...
# ============================================================
#   SIMPLE MINIMAP RENDERING FUNCTION (Option 1)
# ============================================================
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # ======================================================
            #  CALL YOUR MINIMAP FUNCTION AND DRAW IT (TOP-RIGHT)
            # ======================================================
            # make sure your minimap function is named: render_minimap(world, agent)
            # ======================================================
            #  CALL MINIMAP (TOP-RIGHT)
            # ======================================================
            mini_surface = render_minimap(world, agent)

            mini_x = args.width - mini_surface.get_width() - 10
            mini_y = 10
            display.blit(mini_surface, (mini_x, mini_y))

            # ======================================================
            #  SAVE PNG
            # ======================================================
            save_surf = mini_surface.copy()
            pygame.image.save(save_surf, f"minimap_frame_{world.hud.frame}.png")
